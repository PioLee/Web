...
export const productMutations = {
  ...
  [REMOVE_PRODUCT_SUCCESS](state, payload) {
    ...
    state.products = state.products.filter(product => product.name !== productId);
  },
  [ADD_PRODUCT](state) {
    state.showLoader = true;
  },
...
export const cartMutations = {
  ...
  [REMOVE_FROM_CART](state, payload) {
    const {productId} = payload
    state.cart = state.cart.filter(product => product.name !== productId)
    Message({
      message: '恭喜你，成功移除购物车！',
      type: 'success'
    })
...
export const manufacturerMutations = {
  ...
  [REMOVE_MANUFACTURER_SUCCESS](state, payload) {
    ...
    state.manufacturers = state.manufacturers.filter(manufacturer => manufacturer.name !== manufacturerId);
  },
  ...
  [UPDATE_MANUFACTURER_SUCCESS](state, payload) {
    ...
    state.manufacturers = state.manufacturers.map(manufacturer => {
      if (manufacturer.name === newManufacturer.name) {
